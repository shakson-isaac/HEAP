...
#'*Visualize Individual Plots - Including GxE interactions for a Corresponding Specification:*
...
covarSelect <- function(covarType){
...
# Function to create data (entire) for visualization
create_data_viz <- function(protID, PXSdata){
  #Load proteomics data: + more
  omicDS <- GS_struct(protID, PXSdata@UKBprot_df)
  protID <- gsub("-", "_", protID)
  protGS <- paste0(protID,"_GS")
...
# Function to preprocess data (entire) for visualization
preprocess_data_viz <- function(protID, covariates,
                            Data, ordinal_contrast = "treatment",
                            ordinal_names){
  # Setup protID and Genetic Score ID:
  protID <- gsub("-", "_", protID)
  G_var <- paste0(protID,"_GS")
  ...
  #Scale only the omics, genetic score, and covariates:
  rel_columns <- c(protID, G_var, covariates)
  numeric_cols <- sapply(Data[, rel_columns], 
                         function(col) is.numeric(col) && !(all(col %in% c(0, 1)))) #Only continuous feature no binaries
  DG_cont_names <- names(numeric_cols[numeric_cols])
  ...
  #'Scale other environments with continuous features:
  #Only obtain Env variables (NOT ordinal, NOT binary, NOT covariates, NOT genetic score or Omic)
  df <- Data %>% select(!all_of(c("eid", rel_columns, ordinal_names)))
  E_cont_names <- continuous_finder(df)
  ...
  #Scale numeric columns of continuous features::
  mean <- lapply(Data[numeric_col_names], function(x) mean(x,na.rm = T))
  sd <- lapply(Data[numeric_col_names], function(x) sd(x,na.rm = T))
...
# Function to obtain finalized model (entire data) for visualization
GxE_assoc_model_viz <- function(df, protID, covariates, Eid){
  ...
  #Setup Formula:
  pred_var <- c(Eid, G_var, paste(G_var, "*", Eid), covariates)
  formula <- as.formula(paste(c(res_var), paste(pred_var, collapse="+"), sep="~"))
  
  #Subset df:
  df2 <- df %>% select(all_of(c(Eid,res_var,G_var,covariates)))
...
# Function to Obtain Model for E-Prot Pair:
GWISmodel <- function(omicID, Eid, covarType){
  PXSdata = covarSelect(covarType)
  
  protData <- create_data_viz(protID = omicID, PXSdata)
  
  protPP <- preprocess_data_viz(protID = omicID, 
                                covariates = PXSdata@covars_list,
                                Data = protData$entire,
                                ordinal_contrast = "treatment",
                                ordinal_names = protData$ordinalVar)
  GWIS <-  GxE_assoc_model_viz(protPP, omicID, PXSdata@covars_list,
                                    Eid)
  return(GWIS)
}
...
newdata_GWIS <- function(statmodel, protID, protGS, Evar){
  ...
  for(i in names(newlist)){
    if(i == protGS){
      ...
      #if continuous do quantiles:
      #if binary just do 0 and 1 (all levels)
      #if ordered do all levels.
      if(max(df[[i]]) == 1){ #binary
        newlist[[i]] = unique(df[[i]])
      } else if(min(df[[i]]) == 0){ #ordinal
        newlist[[i]] = unique(df[[i]])
      } else { #continuous
...
GWIS_plots <- function(statmodel, Evar, Evar_name, protID, custom_levels = c(),
                       title){
  protID <- gsub("-", "_", protID)
  protGS <- paste0(protID,"_GS")
  protGxEid <- paste0(Evar,":",protGS)
  model <- statmodel
  GxEdf <- model$model
  ...
  newdata$y_pred <- predict(model, newdata, type = "response", interval = "confidence", level=0.95)[,1]
  newdata$lower_ci <- predict(model, newdata, type = "response", interval = "confidence", level=0.95)[,2]
  newdata$upper_ci <- predict(model, newdata, type = "response", interval = "confidence", level=0.95)[,3]
  
  # Convert Evar to a factor if necessary
  if(max(newdata[[Evar]] == 1)){
    newdata[[Evar]] <- as.factor(newdata[[Evar]])
  }
  ...
  gg1 <- newdata %>%
    ...
    # Add 95% CIs
    geom_ribbon(aes(ymin = lower_ci, ymax = upper_ci, fill = .data[[Evar]]), 
                alpha = 0.2, color = NA) +  # Add shaded area for CI
    xlab(paste0(protID, ": Genetic Score")) +
  ...
  if(length(custom_levels) > 0){
    gg2 <- gg1 + scale_color_manual(
      values = scales::hue_pal()(length(custom_levels)),  # Use default ggplot color palette
      labels = custom_levels  # Custom labels
    ) +
      scale_fill_manual(
        values = scales::hue_pal()(length(custom_levels)),  # Use default ggplot fill palette
        labels = custom_levels  # Custom labels
      )
...
# Function to run GWIS plots outside script (when sourcing!)
runGWISplot <- function(omic, exposure, CType,
                        ename, levels = c(), plotname){
  #ALPP and Smoking:
  model <- GWISmodel(omicID = omic, 
                         Eid = exposure, 
                         covarType = CType)
  gg <- GWIS_plots(model, 
             Evar = exposure,
