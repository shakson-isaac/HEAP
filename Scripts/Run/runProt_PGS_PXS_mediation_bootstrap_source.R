...
# Function to wrap all bootstrap mediation analysis for external usage:
runBoot_MDwrap <- function(covarType, protlist, dzlist, n){
  MDload <- readRDS(file = paste0("/n/scratch/users/s/shi872/UKB_intermediate/UKB_MDstore_",
                                  covarType,".rds"))
  DZres <- runBoot_MDmulti(protlist, dzlist, MDload, nsamp = n)
